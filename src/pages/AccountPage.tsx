...
export default function AccountPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setIsEditing(false);
      toast.success('Dine oplysninger er opdateret');
    } catch (error: any) {
    ...
    <div className="store-container py-12 md:py-20">
      <div className="max-w-3xl mx-auto">
        ...
        <div className="text-center mb-12">
          ...
          {isAdmin && (
            <div className="inline-flex items-center gap-1.5 mt-3 px-3 py-1.5 bg-primary/10 text-primary text-xs font-medium rounded-full">
              <Shield size={14} />
              Administrator
            </div>
          )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-12">
          <Link
            to="/konto/ordrer"
            className="flex items-center gap-4 p-6 bg-card border border-border rounded-2xl hover:border-primary/30 hover:shadow-lg transition-all group"
          >
          ...
          >
            <div className="w-12 h-12 bg-secondary rounded-xl flex items-center justify-center group-hover:bg-primary/10 transition-colors">
              <Heart size={22} className="text-muted-foreground group-hover:text-primary transition-colors" />
            </div>
            <div className="flex-1">
          ...
          {isAdmin && (
            <Link
              to="/admin"
              className="flex items-center gap-4 p-6 bg-primary/5 border border-primary/20 rounded-2xl hover:border-primary/40 hover:shadow-lg transition-all group md:col-span-2"
            >
              ...
              <div className="flex-1">
                ...
                {isEditing ? (
                  <input
                    ...
                    className="input-cozy text-sm w-full"
                  />
                ) : (
            ...
            <div className="flex items-start gap-4 py-3">
              ...
              <div className="flex-1">
                ...
                {isEditing ? (
                  ...
                  <p className="text-foreground">
                    {formData.address 
                      ? `${formData.address}, ${formData.zip} ${formData.city}${formData.country ? `, ${formData.country}` : ''}`
                      : 'Ikke angivet'}
                  </p>
                )}
              </div>
